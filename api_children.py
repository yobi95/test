...
def children_status_get():
    ...
    return res
#it can delete,not in use--------------------------------------------    
@api_children.route('/api/children/info_search', methods=['GET'])
def children_infos_get_search():
    vres = verify_auto(request, allows=['admin', 'staff','parent'])
    page = request.args.get('page')
    limit = request.args.get('limit')
    if page is None or limit is None:
...
# data analysis***********************************************************************  
# for deploy data analysis function,but not in use
def data1(df,fig): 
...
def get_status_by_pid_date(tp, startt, endt, sk):
    """
    ...
    if type(endt) == type(''):
        ...
        endt = today.strftime('%Y-%m-%d')
    pid = gv.logged[sk]['id']
    wstr = 'where child_id IN (SELECT child.id FROM child, parent WHERE parent.id = {0} AND FIND_IN_SET(child.id, parent.child_ids))'.format(str(pid))
    #wstr = 'where child_id IN (select child_ids from parent where id = {0})'.format(str(pid))
    if startt is not None:
        ...
        for tbn,fl in _sttb.items():
            if tbn == tp:
                ...
                if tbn == 'skin' or tbn == 'diaper':
                    datas = funcs.get_datas_status_with_name(cur, tbn, fl['flist'], wstr=wstr, logger=gv.logger)
                else:
                    datas = funcs.get_datas_status_with_name(cur, tbn, fl['flist'], wstr=wstr, logger=gv.logger)
...
@api_children.route('/api/children/statuses_parent', methods=['GET'])
def children_statuses_get_parent():
    vres = verify_auto(request, allows=['admin', 'staff','parent'])
    if not vres['code'] == 0:
        # verification failed
        vres['count'] = 0
    ...
    if page is None or limit is None:
        ...
        limit = int(limit)
    tp = request.args.get('type')
    sk = request.args.get('secret_key')
    startt = request.args.get('startt')
    endt = request.args.get('endt')
    #2(have)
    print("tp: " + str(tp))
    ...
    print("Debug Type : " + tp) 
    res = get_status_by_pid_date(tp, startt, endt, sk)
    #5
    return res
#************************************************************************************    
