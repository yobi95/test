...
def insert_parent():
    ...
    fs_u = [
        ...
        {'fn': 'parent_allow', 'type': 'str', 'val': request.form.get('parent_allow')}
    ]
...
def update_staff():
    ...
    return {'code': 0, 'msg': '', 'count': 0, 'data': []}
#**************************************************************************************************************
@api_user.route('/api/users/delete_staff', methods=['GET','POST','DELETE'])
def staff_delete():
    id = request.form.get('id')
    fs_p = [
        {'fn': 'id', 'type': 'int'},
    ...
    tbn_p = 'staff'
    tbn_u = 'user'
    sqlstr = 'DELETE FROM `{tbn}` set {0} where id={1};'
    try:
        conn = gv.dbpool.connection()
        cur = conn.cursor()
        cur.execute("SET NAMES UTF8;")
...
# for child searching function----------------------------------------------------------------------------------------------------------------------------------------------------
@api_user.route('/api/users/search', methods=['GET'])
def searching():
    _param_search_text =  request.args.get('search_text')
    print("DEBUG Lee Lo Mo 2")
    print(_param_search_text)
    tbn = 'child'
    flist = [
        {'fn': 'id', 'type': 'str'},
    ...
    res = funcs.get_datas(cursor, tbn, flist,  logger=gv.logger)
    cursor.close()
    conn.close()
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SET NAMES UTF8;")
        wstr = "where name like '%" + _param_search_text + "%'" + " OR child_no like '%" + _param_search_text + "%'"+ " OR from_base64(name_chi) like '%" + _param_search_text + "%'"
        print(wstr)
        res = funcs.get_datas(cur, tbn, flist, wstr=wstr, logger=gv.logger)
        cur.close()
        conn.close()
        return res
    except:
...
# for parent searching function----------------------------------------------------------------------------------------------------------------------------------------------------    
@api_user.route('/api/users/parent_search', methods=['GET'])
def parent_searching():
    _param_search_text =  request.args.get('search_text')
    print(_param_search_text)
    tbn = 'parent'
    flist_t2 = [
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SET NAMES UTF8;")
        wstr = "where from_base64(name) like '%" + _param_search_text + "%'" + " OR child_ids like '%" + _param_search_text + "%'"+ " OR from_base64(name_chi) like '%" + _param_search_text + "%'"
        print(wstr)
        res = funcs.get_datas(cur, tbn, flist_t2, wstr=wstr, logger=gv.logger)
        cur.close()
        conn.close()
        return res
    except:
...
# for status searching function---------------------------------------------------------------------------------------------------------------------------------------------------- 
@api_user.route('/api/users/status_search', methods=['GET'])
def status_searching():
    _param_search_text =  request.args.get('search_text')
    print(_param_search_text)
    tbn = 'skin'   #,'temperature','meal','nap','diaper','health','perform'
    flist_3 = [
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SET NAMES UTF8;")
        #query = "Select c.id,CONVERT(FROM_BASE64(c.name_chi),CHAR),c.name,CONVERT(FROM_BASE64(vpi.value_chi),CHAR), CONVERT(FROM_BASE64(vpd.value_chi),CHAR),s.child_id,CONVERT(FROM_BASE64(vci.value_chi),CHAR),s.remark,s.staff,s.timestamp from skin s LEFT Join va_condition_id vci on s. condition_id = vci.id LEFT join child c on c.id = s.child_id  LEFT join skin_position sp on sp.id = s.child_id LEFT Join va_position_id vpi on sp.position_id = vpi.id LEFT Join va_position_detail vpd on sp.position_detail = vpd.id"
        fstr = "c.id, CONVERT(FROM_BASE64(c.name_chi),CHAR), c.name, s.child_id, s.remark, s.staff, s.timestamp"
        tbn = "skin s"
        wstr = "LEFT Join va_condition_id vci on s. condition_id = vci.id \
                ...
                " where from_base64(name) like '%" + _param_search_text + "%'" + " OR child_no like '%" + _param_search_text + "%'"+ " OR from_base64(name_chi) like '%" + _param_search_text + "%'"
        print("fstr: " + fstr)
        print("wstr: " + wstr)
        res = funcs.get_datas_status_search(cur,tbn,fstr,flist=flist_3,wstr=wstr,logger=gv.logger)
        cur.close()
        conn.close()
        return res
    except:
